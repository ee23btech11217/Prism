...
module processor(
    ...
    // The one BRAM outside the Shift loop
    input [15:0] ext_bram_data_in,
    output [15:0] ext_bram_data_out,
    input ext_bram_we,
    input [9:0] ext_bram_addr,
    
    // Frame buffer out interface - 128-bit AXI
    input [3:0] fbram_group_sel,  // Select which group of 8 FBRAMs (0-9)
            ...
            case (state)
                ...
                STATE_INTERNAL_SHIFT: begin
                    ...
                    if (shift_counter == 0) begin
                        ...
                        // Handle (2 cycles) for the first address
    ...
    // Frame buffer output collection - 8 cores per group, 10 groups total
    // Each group outputs 128 bits (8 cores x 16 bits)
    wire [15:0] fb_doutb_cores [39:0];
    
    // Frame buffer output mux - select 8 cores based on group_sel
    genvar fb_idx;
    generate
        for (fb_idx = 0; fb_idx < 8; fb_idx = fb_idx + 1) begin : fb_mux_gen
            assign fb_doutb[fb_idx*16 +: 16] = fb_doutb_cores[fbram_group_sel*8 + fb_idx];
        end
    endgenerate
    ...
    generate
        for (i = 0; i < 40; i = i + 1) begin : core_gen
            ...
            wire [9:0] fb_core_addr = 
                ((i / 8) == fbram_group_sel) ? fb_addrb : 10'b0;
            ...
            core #(
                ...
                .chunkID0(chunkID),    // Same chunk ID for all threads
                .chunkID1(chunkID),    // Same chunk ID for all threads
                .rst_prog_mem(rst_prog_mem),
                .prog_wena(prog_write_enable),  // Common write enable for all program memories
                ...
                .fb0_doutb(fb_doutb_cores[i]),
                .fb1_doutb(), 
                .bmp_en(bmp_enable),  // Common enable for all bitmap memories
